...
_FLOW_SCRIPT_DOC = (
    ...
    "           node_placement='NETWORK_SIMPLEX',\n"
    "           thoroughness=7,                  # 1..100, higher = better\n"
    "           elk_options={'elk.x.y': 'val'})  # raw escape hatch\n"
    "      # direction: TB | LR | BT | RL          (top-down default)\n"
    "      # engine:    elk (default, higher quality) | dagre (faster)\n"
    "      # edge_routing: orthogonal (default, polyline w/ 90° bends) |\n"
    "      #               polyline | splines\n"
    "      # ELK-only knobs are ignored by dagre. See ELK reference\n"
    "      # https://eclipse.dev/elk/reference/options.html for the\n"
    "      # full key vocabulary you can pass via elk_options.\n"
    ...
    "            source_side='top|right|bottom|left',  # pin source side\n"
    "            target_side='top|right|bottom|left')  # pin target side\n"
    "      # source_side/target_side: hint ELK orthogonal routing about\n"
    "      # which side of each node to exit/enter. Useful for back-edges\n"
    "      # (e.g. retry loopback should exit LEFT, not TOP).\n"
    "\n"
    "  # Swimlanes:\n"
